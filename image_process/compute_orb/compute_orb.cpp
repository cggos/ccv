...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
    ...
    for (auto &kp : keypoints) {
		...
		int t = kp.pt.y-half_patch_size;
		int b = kp.pt.y+half_patch_size-1;

		if(l<0 || l>=image.cols || t<0 || t>=image.rows || r<0 || r>=image.cols || b<0 || b>=image.rows)
			continue;
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    for (auto &kp: keypoints) {
        ...
        for (int i = 0; i < 256; i++) {
			...
			int px = kp.pt.x + p_offset_x;
			int py = kp.pt.y + p_offset_y;
			int qx = kp.pt.x + q_offset_x;
			int qy = kp.pt.y + q_offset_y;

			if(px<0 || px>=image.cols || py<0 || py>=image.rows || qx<0 || qx>=image.cols || qy<0 || qy>=image.rows){
				d.clear();
				break;
			}
			...
			if(pu<0 || pu>=image.cols || pv<0 || pv>=image.rows || qu<0 || qu>=image.cols || qv<0 || qv>=image.rows){
				d.clear();
				break;
			}
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
	...
	for(int i=0; i<nSizeD1; ++i){
		if(desc1[i].empty())
			continue;
		int d_min = 255;
		int j_min = 0;
		for(int j=0; j<nSizeD2; ++j){
			if(desc2[j].empty())
				continue;
			int dist = 0;
			for(int n=0; n<256; ++n){
