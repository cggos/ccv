...
RotationMatrix rodrigues(const Vector3 &v3) {
  hpc::TScalarF theta = v3.l2norm();
  if (theta == 0) return RotationMatrix();
  Vector3 v3_hat = v3 / theta;
  hpc::TScalarF c1 = std::cos(theta);
  hpc::TScalarF s1 = std::sin(theta);
...
RotationMatrix from_two_vector(const Vector3 &v31, const Vector3 &v32) {
  hpc::TScalarF theta1 = v31.l2norm();
  hpc::TScalarF theta2 = v32.l2norm();
  if (theta1 == 0 || theta2 == 0) return RotationMatrix();

  Vector3 v31_hat = v31 / theta1;
  Vector3 v32_hat = v32 / theta2;
  Vector3 v312 = Matrix::cross(v31_hat, v32_hat);
  hpc::TScalarF s = v312.l2norm();
  if (s == 0) return RotationMatrix();
  Vector3 v312_hat = v312 / s;
  hpc::TScalarF c = v31_hat.dot(v32_hat);
