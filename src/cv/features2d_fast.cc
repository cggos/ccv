...
cg::Point2D<int> FAST::fast_pixel_ring[16] = {
    cg::Point2D<int>(0, 3),  cg::Point2D<int>(1, 3),   cg::Point2D<int>(2, 2),   cg::Point2D<int>(3, 1),
    cg::Point2D<int>(3, 0),  cg::Point2D<int>(3, -1),  cg::Point2D<int>(2, -2),  cg::Point2D<int>(1, -3),
    cg::Point2D<int>(0, -3), cg::Point2D<int>(-1, -3), cg::Point2D<int>(-2, -2), cg::Point2D<int>(-3, -1),
    cg::Point2D<int>(-3, 0), cg::Point2D<int>(-3, 1),  cg::Point2D<int>(-2, 2),  cg::Point2D<int>(-1, 3),
};
...
int FAST::old_style_corner_score(const cg::Image<unsigned char> &img,
                                 cg::Point2D<int> c,
...
void FAST::compute_fast_score_old(const cg::Image<unsigned char> &img,
                                  const std::vector<cg::Point2D<int> > &corners,
                                  int barrier,
                                  std::vector<int> &scores) {
  int pointer_dir[16];
  for (int i = 0; i < 16; i++) pointer_dir[i] = fast_pixel_ring[i].x + fast_pixel_ring[i].y * img.size().width;
...
void FAST::fast_nonmax(const cg::Image<unsigned char> &img,
                       const std::vector<cg::Point2D<int> > &corners,
                       int barrier,
                       std::vector<cg::Point2D<int> > &max_corners) {
  std::vector<int> scores;
...
void FAST::detect_1(const unsigned char *img_data,
                    unsigned int width,
                    unsigned int height,
...
void FAST::detect_2(const unsigned char *img_data,
                    unsigned int width,
                    unsigned int height,
  ...
  int offset[16] = {0};
  for (int k = 0; k < 16; k++) offset[k] = get_bresenmancicle_pos(0, width, k);
