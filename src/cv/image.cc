...
int MatrixRotateImage(int cx, int cy, float degree, float M[3][3]) {
  float A[3][3] = {1, 0, -cx, 0, 1, -cy, 0, 0, 1};

  float dcos = std::cos(degree);
  float dsin = std::sin(degree);
  float B[3][3] = {dcos, dsin, 0, -dsin, dcos, 0, 0, 0, 1};

  float C[3][3] = {1, 0, cx, 0, 1, cy, 0, 0, 1};
...
void ImgProjectiveTransform(const unsigned char *imgSrc,
  ...
  for (int i = 1; i < 4; i++) {
    if (pointsDst[i][0] < minX) minX = pointsDst[i][0];
    if (pointsDst[i][0] > maxX) maxX = pointsDst[i][0];
    if (pointsDst[i][1] < minY) minY = pointsDst[i][1];
    if (pointsDst[i][1] > maxY) maxY = pointsDst[i][1];
  }
  ...
  *imgDst = new unsigned char[sizeDst];
  ...
  for (int y = (int)minY + 1; y <= (int)maxY; ++y) {
    for (int x = (int)minX + 1; x <= (int)maxX; ++x) {
      ...
      xSrc /= zSrc;
      ySrc /= zSrc;
      if (xSrc < 0 || xSrc > widthSrc || ySrc < 0 || ySrc > heightSrc) continue;
  ...
  *imgDst = new unsigned char[sizeDst];
  ...
  for (int y = 0; y < heightDst; y++) {
    for (int x = 0; x < widthDst; x++) {
      ...
      if (xfSrc < 0 || xfSrc > widthSrc || yfSrc < 0 || yfSrc > heightSrc) continue;
