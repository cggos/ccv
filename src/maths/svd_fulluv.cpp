...
void jacobi_svd(
    float *At, size_t astep, float *_W, float *Vt, size_t vstep, int m, int n, int n1, double minval, float eps) {
  ...
  for (iter = 0; iter < max_iter; iter++) {
    ...
    for (i = 0; i < n - 1; i++)
      for (j = i + 1; j < n; j++) {
        ...
        for (k = 0; k < m; k++) p += (double)Ai[k] * Aj[k];

        if (std::abs(p) <= eps * std::sqrt((double)a * b)) continue;
  ...
  for (i = 0; i < n - 1; i++) {
    ...
    if (i != j) {
      std::swap(W[i], W[j]);
      if (Vt) {
        for (k = 0; k < m; k++) std::swap(At[i * astep + k], At[j * astep + k]);

        for (k = 0; k < n; k++) std::swap(Vt[i * vstep + k], Vt[j * vstep + k]);
      }
    }
  }

  for (i = 0; i < n; i++) _W[i] = (float)W[i];
  ...
  for (i = 0; i < n1; i++) {
    ...
    for (int ii = 0; ii < 100 && sd <= minval; ii++) {
      ...
      for (iter = 0; iter < 2; iter++) {
        for (j = 0; j < i; j++) {
          sd = 0;
          for (k = 0; k < m; k++) sd += At[i * astep + k] * At[j * astep + k];
          float asum = 0;
          for (k = 0; k < m; k++) {
          ...
          for (k = 0; k < m; k++) At[i * astep + k] *= asum;
        }
      }
      sd = 0;
      for (k = 0; k < m; k++) {
