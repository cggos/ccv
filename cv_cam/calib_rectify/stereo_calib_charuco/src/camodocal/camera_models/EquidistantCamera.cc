...
void EquidistantCamera::estimateIntrinsics(
    ...
    const std::vector<std::vector<int> > &arucoIds,
  ...
  int board_h = boardSize.height;
  int board_w = boardSize.width;
  ...
  for (size_t i = 0; i < imagePoints.size(); ++i) {
    ...
    std::vector<int> ids_aruco = arucoIds[i];
    std::vector<cv::Point2f> img_points = imagePoints.at(i);

      cv::Mat mat_mask = cv::Mat::zeros(boardSize, CV_8UC1);
      for(int n=0; n<img_points.size(); ++n) {
//          int row = ids_aruco[n] % board_h;
//          int col = ids_aruco[n] / board_h;
        int row = board_h - 1 - ids_aruco[n] / board_w;
        int col = ids_aruco[n] % board_w;
          mat_mask.at<unsigned char>(row, col) = 1;
      }

      std::vector<Eigen::Vector2d> vec_centers;
      std::vector<double> vec_radius;
      for(int h=0; h<mat_mask.rows; ++h) {
          std::vector<cv::Point2d> circles;
          for(int w=0; w<mat_mask.cols; ++w) {
              if(mat_mask.at<unsigned char>(h, w) == 0)
                  continue;
              int idx = board_h*w+h;
              circles.push_back(img_points[idx]);
          }
          if(circles.size()<3)
              continue;
          double center_x = 0.0;
          double center_y = 0.0;
          double radius   = 0.0;
          fitCircle(circles, center_x, center_y, radius);
          vec_centers.push_back(Eigen::Vector2d(center_x, center_y));
...
//        std::vector<cv::Point2f> img_points = imagePoints.at(i);
//        circle.push_back(img_points.at(r * boardSize.width + c));
//      }
//
//      fitCircle(circle, center[r](0), center[r](1), radius[r]);
//    }

    for (int j = 0; j < vec_centers.size(); ++j) {
      for (int k = j + 1; k < vec_centers.size(); ++k) {
        ...
        ipts = intersectCircles(
                vec_centers[j](0), vec_centers[j](1), vec_radius[j],
                vec_centers[k](0), vec_centers[k](1), vec_radius[k]);
        if (ipts.size() < 2) {
          continue;
        }
