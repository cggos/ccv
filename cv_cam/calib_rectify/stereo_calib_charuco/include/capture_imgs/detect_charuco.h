...
public:
    ...
    inline void detect(
        ...
        cv::cvtColor(image_, img_show, COLOR_GRAY2BGR);
        {
            ...
            if (marker_ids_.size() > 0)
                interpolated_corners =
                        aruco::interpolateCornersCharuco(marker_corners_, marker_ids_, image_, charuco_board_,
    ...
    inline void add_corners() {
        if(marker_ids_.size() > 0) {
            ...
            all_imgs_.push_back(image_.clone());
//            static bool is_tmp = true;
//            if(is_tmp) {
//                img_tmp_ = image_.clone();
//                is_tmp = false;
//            }
        }
    }
