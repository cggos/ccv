...
#include <camera_roswrapper/CameraStereoConfig.h>
...
public:
    ...
    typedef camera_roswrapper::CameraStereoConfig Config;
    ...
    CameraDriver()
        ...
        for (size_t i = 0; i < NUM_CAMERAS; ++i) {
            ...
            if (camera[i].open(camera_index[i])) {
                ...
                ROS_ERROR_STREAM(
                        "Failed to open " << CameraString[i] << " camera (index = " << camera_index[i] << ")!");
                ros::shutdown();
            }
        }
...
private:
    ...
    void setCameraInfo(CameraInfoManager &camera_info_manager, const std::string &camera_info_url,
                       std::string &camera_info_url_new) {
