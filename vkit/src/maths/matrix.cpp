...
#define IMIN(a,b) (iminarg1=(a),iminarg2=(b),(iminarg1) < (iminarg2) ? (iminarg1) : (iminarg2))
    ...
    Matrix::Matrix(const Matrix &M) {
        ...
        for (int32_t i = 0; i < M.m_; i++)
            memcpy(val_[i], M.val_[i], M.n_* sizeof(FLOAT));
    }
    ...
    Matrix Matrix::diag(const Matrix &M) {
        if (M.m_ > 1 && M.n_ == 1) {
            Matrix D(M.m_, M.m_);
            for (int32_t i = 0; i < M.m_; i++)
                D.val_[i][i] = M.val_[i][0];
            return D;
        } else if (M.m_ == 1 && M.n_ > 1) {
            Matrix D(M.n_, M.n_);
            for (int32_t i = 0; i < M.n_; i++)
                D.val_[i][i] = M.val_[0][i];
            return D;
        }
    ...
    void Matrix::conservative_resize(int32_t p, int32_t q) {
        ...
        if(p<=m_ && q>n_)
            mat.set_mat(0, 0, get_mat(0, 0, p-1, n_-1));
        if(p>m_ && q<=n_)
            mat.set_mat(0, 0, get_mat(0, 0, m_-1, q-1));
        *this = mat;
    }
    ...
    const Matrix Matrix::operator+(const Matrix &M) const {
        ...
        if (A.m_ != B.m_ || A.n_ != B.n_) {
            cerr << "ERROR: Trying to add matrices of size (" << A.m_ << "x" << A.n_ <<
                 ") and (" << B.m_ << "x" << B.n_ << ")" << endl;
            exit(0);
        }
        Matrix C(A.m_, A.n_);
        for (int32_t i = 0; i < m_; i++)
            for (int32_t j = 0; j < n_; j++)
                C.val_[i][j] = A.val_[i][j] + B.val_[i][j];
        return C;
    }
    ...
    const Matrix Matrix::operator-(const Matrix &M) const {
        ...
        const Matrix &B = M;
        if (A.m_ != B.m_ || A.n_ != B.n_) {
            cerr << "ERROR: Trying to subtract matrices of size (" << A.m_ << "x" << A.n_ <<
                 ") and (" << B.m_ << "x" << B.n_ << ")" << endl;
            exit(0);
        }
        Matrix C(A.m_, A.n_);
    ...
    const Matrix Matrix::operator*(const Matrix &M) const {
        ...
        const Matrix &B = M;
        if (A.n_ != B.m_) {
            cerr << "ERROR: Trying to multiply matrices of size (" << A.m_ << "x" << A.n_ <<
                 ") and (" << B.m_ << "x" << B.n_ << ")" << endl;
            exit(0);
        }
        Matrix C(A.m_, B.n_);
        for (int32_t i = 0; i < A.m_; i++)
            for (int32_t j = 0; j < B.n_; j++)
                for (int32_t k = 0; k < A.n_; k++)
                    C.val_[i][j] += A.val_[i][k] * B.val_[k][j];
        return C;
    }
    ...
    const Matrix Matrix::operator/(const Matrix &M) const {
        ...
        if (A.m_ == B.m_ && A.n_ == B.n_) {
            ...
            for (int32_t i = 0; i < A.m_; i++)
                for (int32_t j = 0; j < A.n_; j++)
                    if (B.val_[i][0] != 0)
                        C.val_[i][j] = A.val_[i][j] / B.val_[i][0];
            return C;
            ...
            for (int32_t i = 0; i < A.m_; i++)
                for (int32_t j = 0; j < A.n_; j++)
                    if (B.val_[0][j] != 0)
                        C.val_[i][j] = A.val_[i][j] / B.val_[0][j];
            return C;

        } else {
            cerr << "ERROR: Trying to divide matrices of size (" << A.m_ << "x" << A.n_ <<
                 ") and (" << B.m_ << "x" << B.n_ << ")" << endl;
            exit(0);
        }
    }
    ...
    bool Matrix::solve(const Matrix &M, FLOAT eps) {
        ...
        if (A.m_ != A.n_ || A.m_ != B.m_ || A.m_ < 1 || B.n_ < 1) {
            cerr << "ERROR: Trying to eliminate matrices of size (" << A.m_ << "x" << A.n_ <<
                 ") and (" << B.m_ << "x" << B.n_ << ")" << endl;
            exit(0);
        }
    ...
    void Matrix::svd(Matrix &U2, Matrix &W, Matrix &V) {
        ...
        for (i = IMIN(m_, n_) - 1; i >= 0; i--) { // Accumulation of left-hand transformations.
            ...
            } else for (j = i; j < m_; j++) U.val_[j][i] = 0.0;
            ++U.val_[i][i];
        }
        ...
        for (k = 0; k < n_; k++) { // flip signs
            s2 = 0;
            for (i = 0; i < m_; i++) if (U.val_[i][k] < 0.0) s2++;
            for (j = 0; j < n_; j++) if (V.val_[j][k] < 0.0) s2++;
            if (s2 > (m_ + n_) / 2) {
