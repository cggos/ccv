...
#ifndef CCV_CV_IMAGE_H
#define CCV_CV_IMAGE_H
...
class Image {
 public:
  ...
  _T &operator()(const int row, const int col) const { return data_[row * size_.width + col]; }

  _T &operator[](const Point2D<int> &pt) const { return data_[pt.y * size_.width + pt.x]; }

  inline _T *data() const { return data_; }

  inline _T *end() const { return &data_[size_.height * size_.width - 1] + 1; }

  inline bool empty() const { return data_ == nullptr; }

  inline Size size() const { return size_; }

  inline unsigned int area() const { return size_.area(); }

  inline bool is_in_image(const Point2D<int> &pt, int border) const {
    return pt.x >= border && pt.x <= (int)size_.width - border && pt.y >= border && pt.y <= (int)size_.height - border;
  }
  ...
  bool copy(Image &img_dst, const Size &size, const Point2D<int> &pt) {
    if (img_dst.size() == Size(0, 0)) return false;
    if (size.width > img_dst.size().width || size.height > img_dst.size().height) return false;
  ...
  bool copy(Image &img_dst, unsigned int width, unsigned int height, const Point2D<int> &pt) {
    if (img_dst.size() == Size(0, 0)) return false;
    if (width > img_dst.size().width || height > img_dst.size().height) return false;
  ...
  void undistort_image(Image &img_dst,
                       unsigned int width,
                       unsigned int height,
                       const CameraModel &model_cam,
                       const DistortModel &model_dis) {
   ...
   * \f]
   */
  inline void sample_interpolation(double x,
                                   double y,
                                   _T &result,
  ...
  int zoom_with_interpolation(Image &img_dst,
                              float scaleX = 1.0f,
                              float scaleY = 1.f,
                              INTERPOLATION_TYPE interpolation_type = INTERPOLATION_BILINEAR) {
  ...
  inline static double *generate_gaussian_template(unsigned int m, double sigma = 0.84089642) {
    ...
    double sumInv = 0.0;
    if (sum > 1e-6) sumInv = 1.0 / sum;
    for (int y = 0; y < m; y++)
      for (int x = 0; x < n; x++) *(gaussian_template + y * m + x) *= sumInv;
...
void ImgProjectiveTransform(const unsigned char *imgSrc,
...
void ImgProjectiveTransform_Fixed(const unsigned char *imgSrc,
                                  unsigned int widthSrc,
                                  unsigned int heightSrc,
                                  float H[3][3],
                                  unsigned char **imgDst,
