#ifndef CCV_CV_Feature2dFAST_H
#define CCV_CV_Feature2dFAST_H

#include <vector>

#include "ccv/cv/features2d.h"
...
class FAST : public Features2D {
 public:
  ...
  static void fast_nonmax(const cg::Image<unsigned char> &img,
 ...
 private:
  ...
  struct collect_pos {
    static inline cg::Point2D<int> collect(const cg::Point2D<int> &pos, int) { return pos; }
  };
  ...
  inline unsigned int get_bresenmancicle_pos(unsigned int pos_base, unsigned int width, unsigned short i) {
    int bresenham_circle[][2] = {{-3, 0}, {-3, -1}, {-2, -2}, {-1, -3}, {0, -3}, {1, -3}, {2, -2}, {3, -1},
  ...
  static int old_style_corner_score(const cg::Image<unsigned char> &img,
                                    cg::Point2D<int> c,
                                    const int *pointer_dir,
                                    int barrier);
  static void compute_fast_score_old(const cg::Image<unsigned char> &img,
                                     const std::vector<cg::Point2D<int> > &corners,
                                     int barrier,
