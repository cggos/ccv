...
#ifndef CCV_CV_VISUAL_TRACKING_H
#define CCV_CV_VISUAL_TRACKING_H
...
inline float get_pixel_value(const cg::YImg8 &img, float x, float y) {
  ...
  if (lx < 0 || ly < 0 || lx + 1 >= img.cols() || ly + 1 >= img.rows()) return 0.f;
  float xx = x - lx;
  float yy = y - ly;
  return float((1 - xx) * (1 - yy) * img(ly, lx) + xx * (1 - yy) * img(ly, lx + 1) + (1 - xx) * yy * img(ly + 1, lx) +
               xx * yy * img(ly + 1, lx + 1));
}

}  // namespace cg

#endif  // CCV_CV_VISUAL_TRACKING_H
