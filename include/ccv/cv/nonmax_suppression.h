...
#ifndef CCV_CV_NONMAX_SUPPRESSION_H
#define CCV_CV_NONMAX_SUPPRESSION_H
...
class NonMaxSuppression {
 ...
 public:
  ...
  static inline void nonmax_suppression_t(const std::vector<cg::Point2D<int> >& corners,
                                          const std::vector<Score>& scores,
                                          std::vector<ReturnType>& nonmax_corners) {
    ...
    for (int i = 0; i < sz; i++) {
      ...
      // Check left
      if (i > 0)
        if (corners[i - 1] == pos - cg::Point2D<int>(1, 0) && Greater::Compare(scores[i - 1], score)) continue;
      ...
      if (pos.y != 0 && row_start[pos.y - 1] != -1) {
        ...
        for (int i = point_above; corners[i].y < pos.y && corners[i].x <= pos.x + 1; i++) {
          int x = corners[i].x;
          if ((x == pos.x - 1 || x == pos.x || x == pos.x + 1) && Greater::Compare(scores[i], score)) goto cont;
        }
      }
      ...
      {
        if (corners[point_below].y < pos.y + 1) point_below = row_start[pos.y + 1];
...
#endif  // CCV_CV_NONMAX_SUPPRESSION_H
