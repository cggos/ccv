...
void CornerDetector::zero_occupancy_grid() { std::fill(occupancy_grid_.begin(), occupancy_grid_.end() - 1, false); }

void CornerDetector::set_grid_position(const cg::Point2f &pos) { occupancy_grid_[sub2ind(pos)] = true; }
...
void CornerDetector::detect_features(const cg::YImg8 &image,
                                     std::vector<cg::Point2f> &features,
...
#ifdef __SSE2__
  fast::fast_corner_detect_10_sse2((fast::fast_byte *)image.data(), image.cols(), image.rows(), image.cols(), 20,
                                   fast_corners);
#else
  fast::fast_corner_detect_10((fast::fast_byte *)image.data, image.cols, image.rows, image.cols, 20, fast_corners);
#endif
  ...
  for (auto it : nm_corners) {
    ...
    if (xy.x >= grid_n_cols_ * grid_width_ || xy.y >= grid_n_rows_ * grid_height_) continue;
    const int k = sub2ind(cg::Point2f(xy.x, xy.y));
    if (occupancy_grid_[k]) continue;
