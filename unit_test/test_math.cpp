...
TEST(maths, Matrix) {
  ...
  mx01 << 1, 2, 3, 4, 5, 6, 7, 8, 0, 1, 2, 3, 6, 7, 8, 9;
...
TEST(math, SVD) {
  ...
  for (int i = 0; i < emx.rows(); ++i)
    for (int j = 0; j < emx.cols(); ++j) cvH.at<float>(i, j) = emx(i, j);
  cv::Mat cvS, cvU, cvVt;
  auto start_time = std::chrono::steady_clock::now();
  cv::SVD::compute(cvH, cvS, cvU, cvVt, cv::SVD::FULL_UV);  // cv::SVD::FULL_UV
  auto end_time = std::chrono::steady_clock::now();
  std::cout << "time OpenCV SVD solve: " << std::chrono::duration<double>(end_time - start_time).count() * 1000 << " ms"
            << std::endl;
  ...
  std::cout << "time Shen SVD solve: " << std::chrono::duration<double>(end_time - start_time).count() * 1000 << " ms"
            << std::endl;
...
TEST(math, ldlt) {
  ...
  auto end_time = std::chrono::steady_clock::now();
  std::cout << "time Eigen ldlt solve: " << std::chrono::duration<double>(end_time - start_time).count() * 1000 << " ms"
            << std::endl;
  std::cout << "LDLT (Eigen): \n" << x1 << std::endl;

  cg::Matrix A(emx.rows(), emx.cols());
  for (int i = 0; i < emx.rows(); ++i)
    for (int j = 0; j < emx.cols(); ++j) A(i, j) = emx(i, j);
  ...
  end_time = std::chrono::steady_clock::now();
  std::cout << "time ldlt solve: " << std::chrono::duration<double>(end_time - start_time).count() * 1000 << " ms"
            << std::endl;
  std::cout << "LDLT:\n" << x << std::endl;
}
